(function () {
  ...
  const replacementAssetRoot = "../assets/sraw-v3";
  const replacementTasks = new Set([4, 8]);
  const practiceAssetRoot = "../assets/sraw-practice-v2";

  function makeTask(taskNumber) {
    const number = String(taskNumber).padStart(2, "0");
    const taskAssetRoot = replacementTasks.has(taskNumber) ? replacementAssetRoot : assetRoot;
    return {
      ...
      referenceSrc: `${taskAssetRoot}/f${formId}/t${number}/input.png`,
      candidates: Array.from({ length: 6 }, (_value, index) => ({
        token: `t${number}-c${index + 1}`,
        src: `${taskAssetRoot}/f${formId}/t${number}/c${String(index + 1).padStart(2, "0")}.png`
      }))
    };
  }
